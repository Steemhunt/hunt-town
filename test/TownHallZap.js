  ...
  describe("Mint", function () {
    describe("Normal cases", async function () {
      beforeEach(async function () {
        await huntToken
          .connect(alice)
          .approve(townHall.address, LOCK_UP_AMOUNT);
      ...
      it("should return correct unlockTime", async function () {
        expect(await townHall.unlockTime(0)).to.equal(
          BigInt(await time.latest()) + LOCK_UP_DURATION
        );
      });

      it("should also be able to call unlockTime from building contract", async function () {
        expect(await building.unlockTime(0)).to.equal(
          await townHall.unlockTime(0)
        );
      });

      it("should decrease Alice's balance by LOCK_UP_AMOUNT", async function () {
        expect(await huntToken.balanceOf(alice.address)).to.equal(
          INITIAL_ALICE_BALANCE - LOCK_UP_AMOUNT
        );
      });

      it("should increase TownHall's balance by LOCK_UP_AMOUNT", async function () {
        expect(await huntToken.balanceOf(townHall.address)).to.equal(
          LOCK_UP_AMOUNT
        );
      });
    ...
    describe("Edge cases", function () {
      it("should revert if alice does not have enough balance", async function () {
        const mintingCount = BigInt(
          parseInt(INITIAL_ALICE_BALANCE / LOCK_UP_AMOUNT)
        );
        await huntToken
          .connect(alice)
          .approve(townHall.address, LOCK_UP_AMOUNT * mintingCount);
        ...
        await expect(
          townHall.connect(alice).mint(alice.address)
      ...
      it("should revert on mintedAt if the token hasn't minted", async function () {
        await expect(townHall.mintedAt(0)).to.be.revertedWithCustomError(
          townHall,
      ...
      it("should revert on unlockTime if the token hasn't minted", async function () {
        await expect(townHall.unlockTime(0)).to.be.revertedWithCustomError(
          townHall,
          "TownHall__InvalidTokenId"
        );
      });
